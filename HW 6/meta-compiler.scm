...
(define (second v lst count)
  (cond ((null? lst) -1)
        ((eq? (car lst) v) count)
        (else (second v (cdr lst) (+ 1 count)))))

(define (first v env count)
  (cond ((null? env) #f)
        ((>= (second v (car env) 0) 0) (list count (second v (car env) 0)))
        (else (first v (cdr env) (+ count 1)))))
